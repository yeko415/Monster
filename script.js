...
async function loadModel() {
  try {
    ...
    if (!response.ok) {
      throw new Error("GLB HTTP " + response.status + " — " + MODEL_PATH);
    }

    const buffer = await response.arrayBuffer();
    console.log("GLB indirildi:", buffer.byteLength, "bytes");
    ...
    loader.parse(buffer, "", gltf => {
      try {
        ...
        showError("Model hazırlanırken hata: " + err.message);
        console.error(err);
      }
    }, err => {
      showError("GLB parse hatası: " + (err?.message || String(err)));
      console.error(err);
    });
  } catch (err) {
    showError("GLB alınamadı: " + (err?.message || String(err)));
    console.error(err);
  }
}
